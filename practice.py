...
def halvesies(numbers):
    ...
    halved = []
    for num in numbers:
        halved.append(float(num / 2.0)) 
        #here I couln't get my division to work so I looked up
        #why it wasn't working and saw that changing one of the numbers to a float
        #would work. I didn't think of it on my own 
    return halved
...
def join_strings_with_comma(words):
    ...
    thing, of course::
        ...
        'Pretzel'
    """
    #I could do it with join but here are two ways:
    # joined = ", ".join(words)
    # return joined
...
def reverse_list_in_place(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    #     for item in items:
    #         popped = items.pop()
    #         items.insert(i, popped)
    #         i += 1
    #         j += 1
    # return items
    
    #I don't understand why this didn't work either
    return items[::-1]
...
def duplicates(items):
    ...
    for item in new_items:
        popped = new_items.pop(0)
        if popped in new_items and popped not in double_items:
            double_items.append(popped)
    ...
    **DO NOT** use the `list.index()` method.
    ...
    for word in words:  
        if letter not in word:
            ...
            for i, my_letter in enumerate(word):
                if my_letter == letter:
                    index_list.append(i)
